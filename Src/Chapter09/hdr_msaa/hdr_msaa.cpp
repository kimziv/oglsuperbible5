...
void HDRMsaa::GenTexCoordOffsets(GLuint width, GLuint height)
{
...
void HDRMsaa::Initialize(void)
{
	...
	windowBatch.Begin(GL_TRIANGLE_FAN, 3, 1);
...
/*
	...
	for (i=0; i<5; i++)
	{
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, brightBlurTextures[i]);
		//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, screenWidth, screenHeight, 0, GL_RGBA, GL_FLOAT, NULL);
        //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, screenWidth/(i*3.0), screenHeight/(i*3.0), 0, GL_RGBA, GL_FLOAT, NULL);
        if(i == 0)
		{
            glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 8,GL_RGB16F, screenWidth, screenHeight, GL_FALSE);
			brightBlurTexturesSize[i][0] = screenWidth;
			brightBlurTexturesSize[i][1] = screenHeight;
		}
        else
		{
            glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 8,GL_RGB16F, screenWidth/(i*3.0), screenHeight/(i*3.0), GL_FALSE);
			brightBlurTexturesSize[i][0] = screenWidth/(i*3.0);
			brightBlurTexturesSize[i][1] = screenHeight/(i*3.0);
		}
	...
	for (i=0; i<4; i++)
	{
		...
		glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, brightBlurTextures[i+1], 0);
		CheckErrors();
	}
...
void HDRMsaa::Resize(GLsizei nWidth, GLsizei nHeight)
{
	...
	hdrTexturesSize[0] = screenWidth;
	hdrTexturesSize[1] = screenHeight;
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, brightBlurTextures[0]);
	//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, screenWidth, screenHeight, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 8, GL_RGB16F, screenWidth, screenHeight, GL_FALSE);
	brightBlurTexturesSize[0][0] = screenWidth;
	brightBlurTexturesSize[0][1] = screenHeight;
	...
	glUseProgram(hdrBloomProg);
	glUniform2iv(glGetUniformLocation(hdrBloomProg, "origImageSize"), 1, hdrTexturesSize);
	glUniform2iv(glGetUniformLocation(hdrBloomProg, "brightImageSize"), 1, brightBlurTexturesSize[0]);
	glUniform2iv(glGetUniformLocation(hdrBloomProg, "blur1Size"), 1, brightBlurTexturesSize[1]);
	glUniform2iv(glGetUniformLocation(hdrBloomProg, "blur2Size"), 1, brightBlurTexturesSize[2]);
	glUniform2iv(glGetUniformLocation(hdrBloomProg, "blur3Size"), 1, brightBlurTexturesSize[3]);
	glUniform2iv(glGetUniformLocation(hdrBloomProg, "blur4Size"), 1, brightBlurTexturesSize[4]);
	...
	projectionMatrix.PushMatrix();
		...
		modelViewMatrix.PushMatrix();
			...
			for (i =0; i<4; i++)
			{
				...
				glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, brightBlurTextures[i]);
				glUniform2iv(glGetUniformLocation(blurProg, "textureUnit0Size"), 1, brightBlurTexturesSize[i]);
				screenQuad.Draw();
			}
